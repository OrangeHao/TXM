...
public class LockPatternUtil {
    ...
    public static String patternToStr(List<LockPatternView.Cell> pattern) {
        if (pattern == null) {
            return null;
        } else {
            int size = pattern.size();
            ...
            return MD5Util.lmsMD5Purity(sb.toString());
        }
    }
    ...
    public static boolean checkPattern2(List<LockPatternView.Cell> pattern, String key) {
        if (pattern == null || key == null) {
            return false;
        } else {
            return patternToStr(pattern).equals(key);
        }
    }
}
